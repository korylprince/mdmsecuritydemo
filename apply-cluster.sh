...
for crd in "${crds[@]}" ; do
    kubectl wait "crd/$crd" --for=create --timeout=60s
...
kubectl wait -n registry deploy/registry-docker-registry --for=create --timeout=60s
...
kubectl wait -n dashboard deploy/dashboard-kong --for=create --timeout=60s
